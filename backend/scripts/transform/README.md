...
| `calibrate_thehyundai_1f.py` | 파일→보고서/파일 | Dabeo→SVG robust 보정, 검증된 물리 scale만 1F 재생성 |
| `calibrate_thehyundai_1f.py` | 파일→보고서/파일 | Dabeo→SVG robust 보정, 검증된 물리 scale만 1F 재생성 |
| `__init__.py` | 패키지 표식 | — |
...
첫 명령은 대응점 잔차와 scale 가설을 `resources/calibration/thehyundai-seoul/1f-report.json`에
재현한다. `--apply`는 calibration의 Naver 동일 꼭짓점과 WGS84 대응점이 모두
`verified`일 때만 그래프·매장·외곽·edge length를 함께 재생성한다.
