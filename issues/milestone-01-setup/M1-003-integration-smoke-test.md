...
- backend API integration test는 `api/tests/integration/`에 둔다.
- frontend app integration test는 `client/integration_test/`에 둔다.
- Playwright E2E는 이 이슈에서 만들지 않고 M5-004의 루트 `e2e/`에서 다룬다.

초기 권장 파일:

```
api/tests/integration/test_buildings_api.py
client/integration_test/buildings_flow_test.dart
```
...
- backend integration test가 `api/tests/integration/`에서 `/buildings` 연동을 검증한다.
- frontend integration test가 `client/integration_test/`에서 건물 목록 로딩/에러 상태를 검증한다.
...
- Playwright는 실제 브라우저에서 frontend web과 backend API를 함께 검증하는 E2E라서 M5-004에서 별도 관리한다.
