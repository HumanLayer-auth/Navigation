...
def test_건물_목록은_요약만_반환한다(db_session):
    ...
    assert result[0]["floors"] == [FLOOR_NAME]
...
def test_검색어로_매장을_조회한다(db_session):
    result = building_queries.search_stores(db_session, BUILDING_ID, "베네타")

    assert result and all("베네타" in store["name"] for store in result)
