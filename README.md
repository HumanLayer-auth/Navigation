...
최단 경로는 서버가 계산하지 않습니다. 클라이언트가 층 지도 응답의 `navigation_graph`(nodes·edges)로 온디바이스 Dijkstra(`client/lib/domain/dijkstra.dart`)를 실행합니다.

현재 앱의 기본 데모 건물은 `thehyundai-seoul`이며, 기본 시드는 Studio 1F 데이터만 적재합니다. API 전체 계약은 서버 실행 뒤 [http://127.0.0.1:8001/docs](http://127.0.0.1:8001/docs)에서 확인할 수 있습니다.
...
- Docker Compose 개발 환경은 컨테이너 시작 command에서 `python -m scripts.seed.reset_and_seed`를 실행합니다.
