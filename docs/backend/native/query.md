...
> **구현 완료 상태.** 경량 검색 두 엔드포인트가 동작하며, 후속으로 Kiwi 정규화와
> `POST /query/ai` FAISS 의미 검색까지 추가됐다. 이 문서는 경량 검색의 계약과 결정 근거를 남긴다.
...
- 완료 조건(Definition of Done):
  ...
  - `POST /query/info`가 질의 텍스트로 `Store`에 적재된 매장·편의시설의
    **위치 정보(층·카테고리 등)**를 반환한다.
  - 매칭 실패·건물 없음·입구 노드 없음 등 실패 조건이 명시된 상태 코드/응답으로 처리된다.

## 2. 범위

- **이번 범위 — 경량 매칭.** 임베딩·RAG 없이 `Store`에 적재된 매장·편의시설의
  **이름·카테고리·동의어**를 텍스트로 매칭한다. 의존성 추가 없음(기존 SQLAlchemy 조회만).
- **초기 범위 밖, 후속 완료 — RAG.** `sentence-transformers` + FAISS 의미 검색은 이 문서의
  응답 계약을 유지한 별도 `POST /query/ai`로 구현됐다([FAISS.md](FAISS.md)).
- **범위 밖 — 경로 계산.** 최단 경로는 서버가 계산하지 않는다. `destination`은 도착 노드(`entrance_node_id`)까지만 주고, Dijkstra는 클라이언트가 수행한다.
- **범위 밖 — RNN 등 밑바닥 시퀀스 학습.** 학습 데이터가 없고 이 문제는 시퀀스 모델링이 아니라 소규모 어휘 검색이다. 경량 매칭으로 부족하면 RNN이 아니라 ② 형태소 정규화(Kiwi 등) → ③ 사전학습 임베딩(RAG) 순으로 올라간다. **둘 다 후속으로 구현됐다** — ③ [FAISS.md](FAISS.md), ② [KIWI.md](KIWI.md).
...
- 응답(성공, 200):
  ...
  - `query_search._to_match()`가 이 계약에 필요한 필드만 조립한다.
...
- **여러 층에 같은 대상이 있으면 층 목록을 준다.** "화장실 몇 층이야?"의 답은 단건 좌표가 아니라 존재하는 층들이다.
  대표 매장과 정규화된 이름이 같은 후보만 모아 `floors: [floor_name, …]`를 반환하며,
  우선순위가 낮은 다른 부분 일치 매장의 층은 섞지 않는다.
  ```json
  {
    "status": "ok",
    "query": "화장실",
...
- 정규화: 앞뒤 공백 제거, 소문자화, 의문형 꼬리("어디야","몇 층") 제거.
  **후속으로 형태소 정규화(Kiwi)가 뒤에 붙었다** — 꼬리 목록에 없는 조사·어미도 떨어진다
  ("화장실이 어디야" → "화장실"). 원문 후보를 먼저 보존하고 문장 끝 `? ! . ,` 등을
  단계적으로 제거해 `A.P.C.` 같은 상호와 자연어 구두점을 함께 처리한다. 설계·측정은 [KIWI.md](KIWI.md).
- 여러 건이 걸리면 **우선순위 → 질의 후보 순서 → `floor.level` → `store.id`**로
  결정적으로 1건을 고른다. AI 경로만 서로 다른 매장명의 tier 2 후보가 여럿이면
  이 ID순 결과를 확정하지 않고 FAISS로 넘긴다.
...
| `text`가 빈 문자열 | `422`(요청 검증). |
| `text`가 공백뿐 | strip 검증 후 `422`. 세 `/query` 엔드포인트가 같은 타입을 쓴다. |
| `text`가 200자를 초과 | `422`. 연속 구두점 후보와 형태소 분석 비용을 요청 하나가 과도하게 키우지 못하게 한다. |
| 질의 끝에 `?`·`!`·`.`·`,` 등 구두점 | 원문과 끝 구두점 제거 후보를 함께 평가. 원문에 포함된 브랜드 구두점은 보존한다. |
| 여러 건 동점 | 4절의 결정적 정렬로 1건 확정(비결정성 금지). |

## 6. 재사용 지점 (기존 코드)

- `app/repositories/building_queries.py`
  - `search_stores(session, building_id, query)` — 기존 이름 부분 일치의 참고 구현. query 엔드포인트는
    별도 `_load_stores()`·`_rank()`로 층·우선순위를 함께 처리한다.
- `app/repositories/query_search.py`
  - `_to_match()` — query 응답 매장 필드 조립.
- `app/repositories/geo_transform.py`
...
구현이 아래를 모두 만족하면 완료로 본다. 테스트는 기존 `backend/tests`의 Given/When/Then·fixture 관례를 따른다. **(초기 경량 검색 구현 당시 전 항목 완료 — 백엔드 66개 테스트 통과, B2 실데이터로 라이브 확인)**
...
- [x] `backend/tests` 단위/통합 테스트 통과. — 초기 범위 당시 66개 통과
...
초기 후속 항목이던 의미 검색은 `POST /query/ai`로 구현 완료됐다([FAISS.md](FAISS.md)).
