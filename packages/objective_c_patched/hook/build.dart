...
String sdkPath(CodeConfig codeConfig) {
  ...
  final sdkRoot = Platform.environment['SDKROOT']?.trim();
  if (sdkRoot != null && sdkRoot.isNotEmpty) {
    final sdkRootDirectory = Directory(sdkRoot);
    if (sdkRootDirectory.isAbsolute && sdkRootDirectory.existsSync()) {
      return sdkRootDirectory.path;
    }
  }

  for (final developerDirectory in _developerDirectoryCandidates(codeConfig)) {
    final resolved = _resolveSdkAlias(
...
Iterable<String> _developerDirectoryCandidates(CodeConfig codeConfig) sync* {
  final configured = Platform.environment['DEVELOPER_DIR']?.trim();
  if (configured != null && configured.isNotEmpty) yield configured;

  final compiler = codeConfig.cCompiler?.compiler.toFilePath();
  if (compiler == null) return;
  final marker = '${Platform.pathSeparator}Toolchains${Platform.pathSeparator}';
  final markerIndex = compiler.indexOf(marker);
  if (markerIndex > 0) yield compiler.substring(0, markerIndex);
}

String? _resolveSdkAlias({
  required String developerDirectory,
  required String target,
  required String? sdkRoot,
}) {
  final platformName = switch (target) {
    'iphoneos' => 'iPhoneOS',
    'iphonesimulator' => 'iPhoneSimulator',
    _ => 'MacOSX',
  };
  final sdkDirectory = Directory(
    '$developerDirectory/Platforms/$platformName.platform/Developer/SDKs',
  );
  if (!sdkDirectory.existsSync()) return null;

  final candidates = sdkDirectory
      .listSync(followLinks: false)
      .whereType<Directory>()
      .where((directory) {
        final name = _directoryName(directory).toLowerCase();
        return name.startsWith(platformName.toLowerCase()) &&
            name.endsWith('.sdk');
      })
      .toList();
  if (candidates.isEmpty) return null;

  final alias = sdkRoot?.toLowerCase();
  if (alias != null && alias.isNotEmpty) {
    final expectedName = alias.endsWith('.sdk') ? alias : '$alias.sdk';
    for (final candidate in candidates) {
      if (_directoryName(candidate).toLowerCase() == expectedName) {
...
String firstLineOfStdout(String cmd, List<String> args) {
  ...
  if (result.exitCode != 0 || lines.isEmpty) {
    throw ProcessException(
      cmd,
      args,
      'Unable to resolve an Apple SDK path. '
